...
const {
  ...
  instanceExpected,
  ssdExpected,
  commitmentTermExpected
} = require('../utils/constants')
const {
  regionExpected,
  monthlyRentExpected,
  mailMonthlyEstimateExpexted
...
describe('End to end test of Google Cloud calculator with the results emailed to temporary email account', function () {
  ...
  it('Opens Google cloud calculator link and adds data to create estimate', async function () {
    ...
    await expect(CalculatorPage.estimateAd).toHaveTextContaining(
      'Compute Engine'
    )
  })
  ...
  it('Checks Instance type is n1-standard-8', async function () {
    ...
    await expect(CalculatorPage.instanceTypeEstimate).toHaveText(
      instanceExpected
    )
  })
  ...
  it('Check commitment term is set to 1 Year', async function () {
    ...
    await expect(CalculatorPage.commitementTermEstimate).toHaveText(
      commitmentTermExpected
    )
  })
  it('Check monthly rent sum is the same with the manual test', async function () {
    ...
    await expect(CalculatorPage.monthlyRentEstimate).toHaveText(
      monthlyRentExpected
    )
  })
  ...
  it('Switches back to mail page and checks email received', async function () {
    ...
    await expect(MailPage.monthlyEstimate).toHaveTextContaining(
      mailMonthlyEstimateExpexted
    )
  })
})
